...
export default function Roles() {
  ...
  const load = async () => {
    try {
      setRoles((await api.get("/hr/roles/get")).data.data || []);
    } catch {
  ...
  const save = async () => {
    ...
    try {
      editing
        ? await api.put(`/hr/roles/update/${editing.id}`, { name })
        : await api.post("/hr/roles/create", { name });
  ...
  const remove = async (role: Role) => {
    ...
    try {
      await api.delete(`/hr/roles/delete/${role.id}`);
    ...
    <div className="container mx-auto space-y-6 p-4 md:p-6 lg:p-8">
        ...
        <CardContent>
          <div className="space-y-2">
            {roles.length ? (
              ...
              <p className="py-10 text-center text-muted-foreground">No roles yet.</p>
            )}
          </div>
        </CardContent>
      </Card>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <Input value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. Manager" />
