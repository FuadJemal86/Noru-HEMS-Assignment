...
type Report = { scheduledShifts: number; attendanceRecords: number; present: number; late: number; absent: number; unrecordedScheduledShifts: number; attendanceRate: number; scheduledHours: number; workedHours: number; recordedEmployees: number };
...
export default function Workforce() {
  ...
  return <div className="container mx-auto space-y-6 p-4 md:p-6 lg:p-8">
    ...
    <Card className="shadow-sm"><CardHeader className="gap-4 sm:flex-row sm:items-end sm:justify-between"><div><CardTitle className="flex items-center gap-2"><FileBarChart className="h-5 w-5" />Workforce report</CardTitle><CardDescription>Scheduled coverage and recorded hours for the selected period.</CardDescription></div><div className="flex flex-wrap gap-2"><Input className="w-auto" type="date" min={minimumDate} value={reportRange.startDate} onChange={(event) => setReportRange({ ...reportRange, startDate: event.target.value })} /><Input className="w-auto" type="date" min={reportRange.startDate || minimumDate} value={reportRange.endDate} onChange={(event) => setReportRange({ ...reportRange, endDate: event.target.value })} /><Button variant="outline" onClick={refreshRange}>Apply</Button></div></CardHeader><CardContent>
      {report && <div className="grid grid-cols-2 gap-3 md:grid-cols-4 xl:grid-cols-5">{[["Shifts", report.scheduledShifts], ["Records", report.attendanceRecords], ["Present", report.present], ["Late", report.late], ["Absent", report.absent], ["Missing records", report.unrecordedScheduledShifts], ["Completion", `${report.attendanceRate}%`], ["Scheduled hrs", report.scheduledHours], ["Worked hrs", report.workedHours]].map(([label, value]) => <div key={String(label)} className="rounded-lg border p-3"><p className="text-xs text-muted-foreground">{label}</p><p className="mt-1 text-xl font-semibold">{value}</p></div>)}</div>}
    </CardContent></Card>
