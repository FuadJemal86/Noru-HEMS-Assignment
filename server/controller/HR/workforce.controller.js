...
const createShift = async (req, res) => {
  try {
    ...
    if (!(await requireEmployee(prisma, employeeId))) return res.status(404).json({ success: false, error: "Employee not found" });

    const shift = await prisma.shift.create({ data: { employeeId, shiftDate: date, startTime, endTime, breakMinutes: Number(breakMinutes), notes: notes?.trim() || null, status }, include: shiftInclude });
...
const getShifts = async (req, res) => {
  try {
    ...
    const shifts = await prisma.shift.findMany({ where, include: shiftInclude, orderBy: [{ shiftDate: "asc" }, { startTime: "asc" }] });
...
const updateShift = async (req, res) => {
  try {
    ...
    const shift = await prisma.shift.update({ where: { id: existing.id }, data, include: shiftInclude });
...
const getAttendance = async (req, res) => {
  try {
    ...
    const attendance = await prisma.attendance.findMany({ where, include: attendanceInclude, orderBy: { date: "desc" } });
...
module.exports = { createShift, getShifts, updateShift, deleteShift, recordAttendance, getAttendance, getWorkforceReport };
